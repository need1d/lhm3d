package com.lhm3d.materialobjects
{
	...
	public class CLTexEnvBumpFresnel3DObject extends Base3DObject
	{
		...
		public function CLTexEnvBumpFresnel3DObject(_envAmount:Number,_textureIndex:int, _textureIndex2:int,_textureIndexBump:int, _vertexLayer:Vector.<Number>, 
														  ...
														  _indexLayer:Vector.<uint>)
		{
			...
			for (var i:int = 0; i < _vertexLayer.length / 3; i++) {
				vertices.push(_vertexLayer[i*3],_vertexLayer[i*3+1],_vertexLayer[i*3+2],_normalLayer[i*3],_normalLayer[i*3+1],_normalLayer[i*3+2], 0.15,_uvLayer[i*2],_uvLayer[i*2+1]);
			}
			
			
			vertexbuffer = Globals.context3D.createVertexBuffer(_vertexLayer.length / 3, 9);
			...
			vertexShaderAssembler.assemble( Context3DProgramType.VERTEX,
				...
				"mov v1, vt0.xyz \n" + // licht-normale (object) in v1
				
				"mov v2, va2\n" + 	// uv's in v2
				
				"m33 vt0.xyz, va1.xyz, vc8\n" + 
				
				"add v3, vt0.xyz, vc13\n" +  //fresnel-factor in v3
				
				"mul vt0.xyz, vt0.xyz, vc12\n"+
				"add v0, vt0.xyz, vc12"	// env-normale (object + view) in v3
		...
		public override function renderWithMatrix(_mat:Matrix3D):void  {
			...
			Globals.context3D.setVertexBufferAt(1, vertexbuffer, 3, Context3DVertexBufferFormat.FLOAT_4); //va1 normal
			Globals.context3D.setVertexBufferAt(2, vertexbuffer, 7, Context3DVertexBufferFormat.FLOAT_2); //va2 (u,v)
			...
			Globals.context3D.setProgramConstantsFromVector(Context3DProgramType.VERTEX, 13,  Vector.<Number>([1.0, 1.0, 1.0, 1.0]));

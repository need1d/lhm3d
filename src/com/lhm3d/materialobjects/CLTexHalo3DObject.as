package com.lhm3d.materialobjects
{
	...
	public class CLTexHalo3DObject extends Base3DObject
	{
		
		private var textureIndex:int;
		
		private var haloHardness:Number;
		private var haloAmount:Number;
		
		
		
		public function CLTexHalo3DObject(_haloHardness:Number,_haloAmount:Number,_textureIndex:int, _vertexLayer:Vector.<Number>, 
									  _normalLayer:Vector.<Number>,_uvLayer:Vector.<Number>,
									  _indexLayer:Vector.<uint>)
		{
			super();
			
			haloHardness = _haloHardness;
			haloAmount = _haloAmount;
			...
			vertexShaderAssembler.assemble( Context3DProgramType.VERTEX,
				...
				"add vt0.xyz, vt0.xyz, vc13.x\n" +  
				"pow vt0.xyz, vt0.xyz, vc13.y\n" +
				"mul v4, vc14, vt0.z\n" + //halo-color in v4

				"mov v2, va2"			// uv's in v2
			);			
		...
		public override function renderWithMatrix(_mat:Matrix3D):void  {
			...
			Globals.context3D.setProgramConstantsFromMatrix(Context3DProgramType.VERTEX, 8, _mvv, true);
			
			Globals.context3D.setProgramConstantsFromVector(Context3DProgramType.VERTEX, 13,  Vector.<Number>([1.0, haloHardness, 0.0, 0.0]));
			Globals.context3D.setProgramConstantsFromVector(Context3DProgramType.VERTEX, 14,  Vector.<Number>([light.haloColor[0]*haloAmount, 
																												  light.haloColor[1]*haloAmount, 
																												  light.haloColor[2]*haloAmount, 1.0]));
			...
			Globals.context3D.setProgramConstantsFromVector(Context3DProgramType.FRAGMENT, 6, Vector.<Number>([1.0, 1.0, 1.0, 0.0])); //for the max value
